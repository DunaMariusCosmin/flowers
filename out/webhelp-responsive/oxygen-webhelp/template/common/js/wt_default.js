...
$(document).ready(function () {
    ...
    handleSideTocPosition();
    $(window).scroll(function(){
        handleSideTocPosition();
    });

    $(window).resize(function(){
        $("#wh_side_toc").removeAttr('style');
        handleSideTocPosition();
    });
 ...
 */
function handleSideTocPosition() {
    var $sideToc = $("#wh_side_toc");
    ...
    if ($slideSection.length > 0){
        ...
        var tocXNav = parseInt($slideSection.offset().left) + parseInt($slideSection.width()) + parseInt($slideSection.css('padding-left')) + parseInt($slideSection.css('padding-right')) + 1;
        var tocWidth =  $sideToc.width() + parseInt($sideToc.css('padding-left')) + parseInt($sideToc.css('padding-right'));
        ...
        if (tocHeight < $slideSection.height()) {
            if ( parseInt(minVisibleOffset-33) <= $(window).scrollTop() ) {
                $sideToc.css("top", "20px").css("position", "fixed").css("left", tocXNav+"px").css("width", tocWidth+"px");
            } else {
